...
import { jiaheBusinessOnboardingSeed } from '../exams/jiahe-business-onboarding'
...
async function upsertCategory(): Promise<number> {
  const category = jiaheBusinessOnboardingSeed.category
...
async function upsertQuestionSet(categoryId: number, set: { questionSet: any; questions: any[] }): Promise<number> {
  const totalQuestions = set.questions.length

  const existing = await executeCompatibleSingle<{ id: number }>(
    'SELECT id FROM question_sets WHERE name = ? AND category_id = ? LIMIT 1',
    [set.questionSet.name, categoryId]
  )
  ...
  if (existing?.id) {
    ...
    await questionSetDB.updateQuestionSet(existing.id, {
      ...
      description: set.questionSet.description || null,
      category_id: categoryId,
      total_questions: totalQuestions,
      is_active: set.questionSet.is_active ?? true,
      allow_view_score: set.questionSet.allow_view_score ?? true
    })
    return existing.id
  }
...
export async function seedJiaheBusinessOnboardingExam() {
  await ensureSchema()

  const categoryId = await upsertCategory()
  // eslint-disable-next-line no-console
  console.log(`类别已准备: ${jiaheBusinessOnboardingSeed.category.name} (id=${categoryId})`)

  for (const set of jiaheBusinessOnboardingSeed.questionSets) {
    const setId = await upsertQuestionSet(categoryId, set)
    // eslint-disable-next-line no-console
    console.log(`题库已准备: ${set.questionSet.name} (id=${setId})`)
    ...
    // eslint-disable-next-line no-console
    console.log(`✓ 导入完成：${inserted}/${set.questions.length} 题`)
  }
...
if (isExecutedDirectly) {
  seedJiaheBusinessOnboardingExam().catch((e) => {
    // eslint-disable-next-line no-console
    console.error(e)
    process.exit(1)
  })
